...
public class MainGui : MonoBehaviour {
	...
	string QuicksavePath;  //System.IO.Directory.GetCurrentDirectory();
	string QuicksavePathHeight;
	string QuicksavePathDiffuse;
	string QuicksavePathNormal;
	string QuicksavePathMetallic; 
	string QuicksavePathSmoothness;
	string QuicksavePathEdge;
	string QuicksavePathAO;
	string QuicksavePathProperty;

#region [blue] 

	string TheSavePath(string inFileName) {
		string thePath;
		string docsPath = Application.dataPath + "/../";
		//My Documents Path
		// if (Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor) {
		//    string docsPath = Environment.GetFolderPath (Environment.SpecialFolder.MyDocuments);	//Windows
		// } 	
		thePath = CombinePath("file:///", docsPath, "Output", inFileName);
		return thePath;
	}
	...
	string TheSavePath() {
		string thePath;
		string docsPath = Application.dataPath + "/../";	
		thePath = CombinePath("file:///", docsPath, "Output");
		return thePath;
	}

	string CombinePath(string first, params string[] others) {
...
#region [green] 	
	...
	public Texture singleIndicator;
	public Texture folderIndicator;
	public GameObject massConversionIndicatorGo;
	...
	void Start () {
		...
		string QuicksavePath = TheSavePath();  //System.IO.Directory.GetCurrentDirectory();
		string QuicksavePathHeight = TheSavePath();
		string QuicksavePathDiffuse = TheSavePath();
		string QuicksavePathNormal = TheSavePath();
		string QuicksavePathMetallic = TheSavePath(); 
		string QuicksavePathSmoothness = TheSavePath();
		string QuicksavePathEdge = TheSavePath();
		string QuicksavePathAO = TheSavePath();
		string QuicksavePathProperty = TheSavePath();
	...
	void OnGUI () {
		...
		// if (GUI.Button (new Rect(Screen.width - 150, 60, 140, 30), "ProcessAll")) {
		// 	ProcessAllMaps();
		// }
		// GUI.enabled = true;
		
		if (_DiffuseMapOriginal == null && _DiffuseMap == null) { GUI.enabled = false; } else { GUI.enabled = true; }
		if (GUI.Button (new Rect(180, Screen.height - 100, 120, 90), "Process H N S Ao")) {
			Process_HNSA();
		}
		GUI.enabled = true;

		if (GUI.Button (new Rect(310, Screen.height - 100, 120, 90), "Folder H N S Ao")) {
			ProcessFolder_HNSA();
		}
		GUI.enabled = true;

		GUI.Label(new Rect(320, Screen.height - 120, 60, 50), "File: " + currFileIndex);
		GUI.Label(new Rect(370, Screen.height - 120, 60, 50), " /    " + totalFileIndexes);

		if(showSingleIndicator == true)
			GUI.DrawTexture(new Rect(190, Screen.height - 170, 100, 20), singleIndicator);

		if(showFolderIndicator == true)	
			GUI.DrawTexture(new Rect(320, Screen.height - 170, 100, 20), folderIndicator);
		...
		//BiggerOpen
		if (GUI.Button (new Rect(50, Screen.height - 100, 120, 90), "OpenDiffuse")) {
			mapTypeToLoad = MapType.diffuseOriginal;
			SetFileMaskImage();
			fileBrowser.ShowBrowser( "Open Diffuse Map", this.OpenFile );
		}
...
#region [blue] 	// PROCESS WHOLE FOLDER

	bool showSingleIndicator = true;
	bool showFolderIndicator = true;

	void ProcessFolder_HNSA() {
		SetFileMaskImage();
		fileBrowser.ShowBrowser_All( "Open Diffuses Folder Batch", this.OpenFolder);
	}

	public List<T_BatchFile> batchFiles = new List<T_BatchFile>();
	public List<Texture2D> folderDiffuseTexs = new List<Texture2D>();
	void OpenFolder (List<T_BatchFile> pathsToFiles) {
		//massConversionIndicatorGo.SetActive(false);
		showSingleIndicator = false;
		showFolderIndicator = false;
		batchFiles.Clear();
		batchFiles = pathsToFiles;
		for (int i = 0; i < pathsToFiles.Count; i++) {
			...
			// clear the existing texture we are loading
			ClearTexture(mapTypeToLoad);
			folderDiffuseTexs.Clear();
		}
		StartCoroutine(ProcessFolderTextures());
	}

	int currFileIndex = 0;
	int totalFileIndexes = 0;
	bool processHNSAFolderBusy = false;
	IEnumerator ProcessFolderTextures() {
		totalFileIndexes = batchFiles.Count;
		for (int i = 0; i < batchFiles.Count; i++) {
			currFileIndex = i + 1;
			mapTypeToLoad = MapType.diffuseOriginal;
			SetFileMaskImage();
			fileBrowser.diffuseFileName = batchFiles[i].fileName;
			ClearTexture(mapTypeToLoad);
			processHNSAFolderBusy = true;
			StartCoroutine ( SaveLoadProjectScript.LoadTexture( mapTypeToLoad, batchFiles[i].path, true ) );
			while( processHNSAFolderBusy ){ yield return new WaitForSeconds (0.1f); }
		}
		currFileIndex = 0;
		totalFileIndexes = 0;
		showSingleIndicator = true;
		showFolderIndicator = true;
		processHNSAFolderBusy = false;
	}

	public void DoProcessFolder_HNSA() {
		//massConversionIndicatorGo.SetActive(false);
		showSingleIndicator = false;
		showFolderIndicator = false;
		CloseWindows();
		FixSize();
		wait = new WaitForSeconds(0.35f);
...
#region [teal]	//MAIN BULK PROCESS
	...
	WaitForSeconds wait;
	IEnumerator DoCommands() {
		CloseWindows();
		HeightFromDiffuseGuiObject.SetActive(true);
		yield return new WaitForSeconds (0.1f);
		HeightFromDiffuseGuiScript.InitializeTextures();
		yield return new WaitForSeconds (0.1f);
		StartCoroutine( HeightFromDiffuseGuiScript.ProcessDiffuse() );
		while( HeightFromDiffuseGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		StartCoroutine( HeightFromDiffuseGuiScript.ProcessHeight() );
		while( HeightFromDiffuseGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		HeightFromDiffuseGuiScript.Close();
		...
		NormalFromHeightGuiObject.SetActive(true);
		yield return new WaitForSeconds (0.1f);
		NormalFromHeightGuiScript.InitializeTextures();
		yield return new WaitForSeconds (0.1f);
		StartCoroutine( NormalFromHeightGuiScript.ProcessHeight() );
		while( NormalFromHeightGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		StartCoroutine( NormalFromHeightGuiScript.ProcessNormal() );
		while( NormalFromHeightGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		NormalFromHeightGuiScript.Close();;
		...
		SmoothnessGuiObject.SetActive(true);
		yield return new WaitForSeconds (0.1f);
		SmoothnessGuiScript.InitializeTextures();
		yield return new WaitForSeconds (0.1f);
		StartCoroutine( SmoothnessGuiScript.ProcessBlur() );
		while( SmoothnessGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		StartCoroutine( SmoothnessGuiScript.ProcessSmoothness() );
		while( SmoothnessGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		SmoothnessGuiScript.Close();
		...
		AOFromNormalGuiObject.SetActive(true);
		yield return new WaitForSeconds (0.1f);
		AOFromNormalGuiScript.InitializeTextures();
		yield return new WaitForSeconds (0.1f);
		StartCoroutine( AOFromNormalGuiScript.ProcessNormalDepth() );
		while( AOFromNormalGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		StartCoroutine( AOFromNormalGuiScript.ProcessAO() );
		while( AOFromNormalGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
		AOFromNormalGuiScript.Close();
		...
		StartCoroutine(SaveTheHNSA());
	}

#endregion

#region [forest]  //SAVING

	string fixDiffusePath;
	string savePath = "";
	public IEnumerator SaveTheHNSA() {
		fixDiffusePath = fileBrowser.diffuseFileName.Replace(".png", "");

		savePath = TheSavePath(String.Concat(fixDiffusePath,"_h"));
		StartCoroutine( SaveLoadProjectScript.SaveTexture ( "png", _HeightMap, savePath) );
		while( SaveLoadProjectScript.busy ){ yield return new WaitForSeconds( 0.01f ); }

		savePath = TheSavePath(String.Concat(fixDiffusePath,"_n"));
		StartCoroutine( SaveLoadProjectScript.SaveTexture ( "png", _NormalMap, savePath) );
		while( SaveLoadProjectScript.busy ){ yield return new WaitForSeconds( 0.01f ); }

		savePath = TheSavePath(String.Concat(fixDiffusePath,"_s"));
		StartCoroutine( SaveLoadProjectScript.SaveTexture ( "png", _SmoothnessMap, savePath) );
		while( SaveLoadProjectScript.busy ){ yield return new WaitForSeconds( 0.01f ); }

		savePath = TheSavePath(String.Concat(fixDiffusePath,"_ao"));
		StartCoroutine( SaveLoadProjectScript.SaveTexture ( "png", _AOMap, savePath) );
		while( SaveLoadProjectScript.busy ){ yield return new WaitForSeconds( 0.01f ); }
		
		yield return wait;
		//massConversionIndicatorGo.SetActive(true);
		showSingleIndicator = true;
		processHNSAFolderBusy = false;
		if(totalFileIndexes == 0)
			showFolderIndicator = true;
	}
...
// 		FixSize();
// 		wait = new WaitForSeconds(0.35f);
// 		StartCoroutine(DoCommandsAll());	
// 	}

// 	IEnumerator DoCommandsAll() {
// 		CloseWindows();
// 		HeightFromDiffuseGuiObject.SetActive(true);
// 		yield return new WaitForSeconds (0.1f);
// 		HeightFromDiffuseGuiScript.InitializeTextures();
...
// 		MetallicGuiObject.SetActive(true);
// 		yield return new WaitForSeconds (0.1f);
// 		MetallicGuiScript.InitializeTextures();
// 		yield return new WaitForSeconds (0.1f);
// 		StartCoroutine( MetallicGuiScript.ProcessBlur() );
// 		while( MetallicGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
// 		StartCoroutine( MetallicGuiScript.ProcessMetallic() );
// 		while( MetallicGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
// 		MetallicGuiScript.Close();
...
// 		EdgeFromNormalGuiObject.SetActive(true);
// 		yield return new WaitForSeconds (0.1f);
// 		EdgeFromNormalGuiScript.InitializeTextures();
// 		yield return new WaitForSeconds (0.1f);
// 		StartCoroutine( EdgeFromNormalGuiScript.ProcessNormal() );
// 		while( EdgeFromNormalGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
// 		StartCoroutine( EdgeFromNormalGuiScript.ProcessEdge() );
// 		while( EdgeFromNormalGuiScript.busy ){ yield return new WaitForSeconds (0.1f); }
// 		EdgeFromNormalGuiScript.Close();

// 		CloseWindows();
// 		yield return wait;
// 		StartCoroutine(SaveAllTheFiles());
// 	}


// 	IEnumerator SaveAllTheFiles() {
// 		yield return wait;
// 		textureToSave = _HeightMap;
// 		mapType = "_h";
// 		SaveFile(QuicksavePathProperty + "_h");

// 		yield return wait;
// 		textureToSave = _NormalMap;
// 		mapType = "_n";
// 		SaveFile(QuicksavePathProperty + "_n");

// 		yield return wait;
// 		textureToSave = _MetallicMap;
// 		mapType = "_m";
// 		SaveFile(QuicksavePathProperty + "_m");

// 		yield return wait;
// 		textureToSave = _SmoothnessMap;
// 		mapType = "_s";
// 		SaveFile(QuicksavePathProperty + "_s");

// 		yield return wait;
// 		textureToSave = _EdgeMap;
// 		mapType = "_edge";
// 		SaveFile(QuicksavePathProperty + "_edge");

// 		yield return wait;
// 		textureToSave = _AOMap;
// 		mapType = "_ao";
// 		SaveFile(QuicksavePathProperty + "_ao");
