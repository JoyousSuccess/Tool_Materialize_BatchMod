...
public class FileBrowser : MonoBehaviour {
	...
	public delegate void FinishedCallbackAll(List<T_BatchFile> paths);
...
#region [green] 
	...
	public void ShowBrowser_All(string title, FinishedCallbackAll callbackAll) {
		wholeFolder = true;
		string dir = this.defaultDir == "" ? Directory.GetCurrentDirectory() : this.defaultDir.FixPathSeparators();
		...
		this.ShowBrowser_All(title, callbackAll, dir, screenRect, "SelectAll");
	}

	public void ShowBrowser_All(string title, FinishedCallbackAll callbackAll, string startingDir, Rect screenRect, string okString)
	{
		if (this.rememberLastDir && this.lastOkDirectory != "") {
			startingDir = this.lastOkDirectory;
		}
	...
	private FinishedCallbackAll callbackAll;
	...
	void GuiWindow( int windowID ){
		...
		if (GUILayout.Button(this.okButtonString, GUILayout.Width(80)) || ( enterPressed && GUI.enabled ) ) {
			this.soundPreviewer.Disable();
			if(wholeFolder == false) {
				this.StartCoroutine(this.FileSelected());
			}
			else {
				this.StartCoroutine(this.AllFilesSelected());
			}
		}
		GUI.enabled = true;
		GUILayout.Space(20);
	...
	public string diffuseFileName = "diffuseFile";
	IEnumerator FileSelected()
	{
		...
		if (this.saveMode) {
			...
			if (this.selectedFile > -1) {
				...
				diffuseFileName = this.files[this.selectedFile];
...
#region [green] 
	...
	List<T_BatchFile> finalNames = new List<T_BatchFile>();

	IEnumerator AllFilesSelected()
	{
		string saveName     = "";
		string finalName    = "";
		string saveShotName = "";

		T_BatchFile batchFile;

		finalNames.Clear();
		for (int i = 0; i < files.Length; i++) {
			batchFile = new T_BatchFile();
			if (this.selectedFile > -1) {
				batchFile.path = Path.Combine(this.currentDirectory, this.files[i]);  //this.selectedFile
				batchFile.fileName = this.files[i].Replace(".png", "");

			} else if (this.typedFilename.Length > 0) {
				batchFile.path = Path.Combine(this.currentDirectory, this.typedFilename);
				batchFile.fileName = this.typedFilename.Replace(".png", "");

			} else if (this.selectedDirectory > -1) {
				batchFile.path = Path.Combine(this.currentDirectory, this.directories[this.selectedDirectory]);
				batchFile.fileName = this.directories[this.selectedDirectory];
			} else {
				batchFile.path = this.currentDirectory;
				batchFile.fileName = this.currentDirectory;
			}

			try {
				batchFile.path = Path.GetFullPath(batchFile.path);
			} catch (ArgumentException) {}

			finalNames.Add(batchFile);
		}
		...
		this.SelectAllConfirmed(finalNames, saveShotName);
	}


	void SelectAllConfirmed(List<T_BatchFile> finalNames, string saveShotName) {

		this.lastOkDirectory = Path.GetDirectoryName(finalNames[0].path);
		string finalNamesZero = finalNames[0].path;
		if (this.relativizePaths) RelativizatePath(ref finalNamesZero);

		this.callbackAll(finalNames);
		this.Close();
	}
